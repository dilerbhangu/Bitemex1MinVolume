...
def get_volume_data(client):
    ...
    dfeth = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='ETHUSD', count=10, partial=True).result()[0]

    dftrx = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='TRX', count=10, partial=True).result()[0]

    dfada = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='ADA', count=10, partial=True).result()[0]

    dfbch = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='BCH', count=10, partial=True).result()[0]

    dfeos = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='EOS', count=10, partial=True).result()[0]

    dfltc = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='LTC', count=10, partial=True).result()[0]

    dfxrp = client.Trade.Trade_getBucketed(
        binSize='5m', reverse=True, symbol='XRP', count=10, partial=True).result()[0]
...
#         msg = 'ETHUSD: 5 min: Volume 10 times'
#         slack_msg(msg)
